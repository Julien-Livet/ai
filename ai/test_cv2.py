...
img_dest = cv2.cvtColor(img_src, cv2.COLOR_RGB2GRAY)
img_dest = cv2.flip(img_dest, 0)

brain.clear_connections()
brain.activate_all_modules()

neuronIds = cv2s.add_value(brain, img_src, "img")

answers = brain.learn(img_dest, depth = 10, compact_name = "img", compact_module = "cv2.constants", module = "cv2.functions", timeout = 30 * 1000)
